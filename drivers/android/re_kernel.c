...
int send_netlink_message(char *msg, uint16_t len)
{
	...
	if (!skbuffer) {
		pr_info("[re_kernel]: netlink alloc failure\n");
		return -1;
	}

	nlhdr = nlmsg_put(skbuffer, 0, 0, netlink_unit, len, 0);
	if (!nlhdr) {
		pr_info("[re_kernel]: nlmsg_put failure\n");
		nlmsg_free(skbuffer);
		return -1;
	}
...
static int __init rekernel_init(void)
{
	...
	if (rekernel_netlink == NULL) {
		pr_info("[re_kernel]: Failed to create server!\n");
		return -1;
	}

	pr_info("[re_kernel]: Create server! NETLINK UNIT: %d\n", netlink_unit);
	rekernel_dir = proc_mkdir("rekernel", NULL);
	if (!rekernel_dir) {
		pr_info("[re_kernel]: Create /proc/rekernel failed\n");
	} else {
		char buff[32];
		...
		if (!rekernel_unit_entry)
			pr_info("[re_kernel]: create rekernel unit failed\n");
	}
